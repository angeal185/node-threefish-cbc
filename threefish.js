...
Utf8Utils = require('./utils/Utf8Utils');
...
function Threefish() {
  ...
  function encCBC(keyBytes, tweakBytes, plaintextBytes, encode){
      var key = bytesToInt64Array(arrayFillTo1024Bits(stringToBytes(keyBytes)));
      var tweak = bytesToInt64Array(tweakBytes);
      var inpBlocks = bytesToInt64Blocks(stringToBytes(plaintextBytes));
      ...
      var result = Buffer.from(int64BlocksToBytes(encBlocks));
      return result.toString(encode);
  }

  function decCBC(keyBytes, tweakBytes, encryptedBytes, encode){
      var encryptedBytes = Buffer.from(encryptedBytes, encode);
      var key = bytesToInt64Array(arrayFillTo1024Bits(stringToBytes(keyBytes)));
      var tweak = bytesToInt64Array(tweakBytes);
      ...
      var result = int64BlocksToBytes(decBlocks);
      return bytesToString(result)
  }

  return {
    enc: function(keyBytes, tweakBytes, plaintextBytes, encode){
      return encCBC(keyBytes, tweakBytes, plaintextBytes, encode);
    },
    dec: function(keyBytes, tweakBytes, encryptedBytes, encode){
      return decCBC(keyBytes, tweakBytes, encryptedBytes, encode);
    }
  }
};
